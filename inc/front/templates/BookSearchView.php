<div id="lbs_books_search">
    <form method="get" name="lbs_search_form" id="lbs_search_form">
            ...
            <table id="lbs_search_form">
                        ...
                        <select name="lbs_book_publisher" id="lbs_book_publisher" class="lbs_select">
							...
							if ( $publishers ) {
								...
								foreach ( $publishers as $publisher ) {
									?>
                                    <option value="<?php echo $publisher->term_id ?>" <?php echo ( $publisher->term_id == $search_publisher ) ? 'selected'
                ...
                <tr>
                    ...
                    <td colspan="3">
						...
						$start_range
							= $min_price
							= $wpdb->get_var( "SELECT min(meta_value + 0) FROM {$wpdb->posts} LEFT JOIN {$wpdb->postmeta} ON {$wpdb->posts}.ID = {$wpdb->postmeta}.post_id WHERE meta_key ='lbs_price_field'" );
						
						$end_range
							= $max_price
							= $wpdb->get_var( "SELECT max(meta_value + 0) FROM {$wpdb->posts} LEFT JOIN {$wpdb->postmeta} ON {$wpdb->posts}.ID = {$wpdb->postmeta}.post_id WHERE meta_key ='lbs_price_field'" );
                        ...
                        <input type="hidden" data-start="<?php echo $start_range ?>" name="lbs_min_value"
                               id="lbs_min_value" value="<?php echo $min_price ?>"> <input type="hidden" data-end="<?php echo $end_range ?>" name="lbs_max_value" id="lbs_max_value"
                                                                                           value="<?php echo $max_price ?>"> <label class="start"> <small><strong>Min:</strong> <span
                                    id="slider-range-value1"></span></small> </label> <label class="end"> <small><strong>Max:</strong> <span id="slider-range-value2"></span></small> </label>
                    </td>
        ...
        <div class="lbs_books_search_footer">
            <input type="hidden" name="current_page_url" value="<?php echo $wp->request ?>"> <input type="hidden" name="paged" id="pagination_page_no"
                                                                                                    value="<?php echo( get_query_var( 'paged' ) + 1 ) ?>">
